...
function createFeed() {
  ...
  // Safety-net polling. When WS is up, it's effectively a no-op (the
  // newHeads handler usually fires first) — when WS is down, this is
  // the only source of updates.
  readOracle();
  setInterval(readOracle, 5_000);

  // Newheads-driven reads via WS. Self-contained connection here — the
  // poller's ws-client.js doesn't support newHeads, and re-using it
  // would mean the dashboard process (which imports this feed) needs
  // a WS connection too. Cheap connection, one frame per block.
