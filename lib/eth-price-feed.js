// ETH/USD price from Chainlink oracle on Ethereum mainnet via Alchemy.
// Primary: WebSocket subscription to AnswerUpdated events (zero polling cost).
// Fallback: HTTP polling every 10s if WS is unavailable.
const ALCHEMY_KEY = '_6LklDOIdb-BwasmACeVlECBsojbFHB5';
const WS_URL = `wss://eth-mainnet.g.alchemy.com/v2/${ALCHEMY_KEY}`;
  ...
  function connectWs() {
    ...
    ws.addEventListener('open', () => {
      ...
      ws.send(JSON.stringify({
        ...
        params: ['logs', { address: ORACLE, topics: [ANSWER_UPDATED] }],
      }));
      // keepalive
