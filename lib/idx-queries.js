...
export async function getDexPriceHistory(hours = 24, unit = 3600) {
  ...
  const rows = await db()`
    SELECT
      ...
      COUNT(*)                                        AS trades
    FROM v_dex_swaps
    WHERE ts >= ${since} AND price_usdm_per_dirty IS NOT NULL
...
export async function getDexRecentSwaps(limit = 50) {
  const rows = await db()`
    SELECT ts, tx_hash, recipient,
           dirty_sold, usdm_gross, usdm_fee, usdm_net, price_usdm_per_dirty
    FROM v_dex_swaps ORDER BY block_num DESC LIMIT ${limit}`;
  return rows.map(r => ({
    ts:              Number(r.ts),
    hash:            r.tx_hash,
...
export async function getIdxVaultCycles(limit = 100) {
  const rows = await db()`
    SELECT ts, tx_hash, distributed, cumulative_total
    FROM v_vault_cycles ORDER BY block_num DESC LIMIT ${limit}`;
  return rows.map(r => ({
    ts:             Number(r.ts),
    hash:           r.tx_hash,
...
export async function getIdxPlayerTrades(owner, limit = 200) {
...
export async function getIdxFactoryStats() {
  const [companies] = await db()`
    SELECT COUNT(*) AS cnt FROM idx_contracts WHERE type = 'company'`;
...
export async function getCompanyCreationHistory(unit = 86400) {
  const [first] = await db()`
    SELECT MIN(first_block) AS b FROM idx_contracts WHERE type = 'company'`;
  if (!first?.b) return [];
...
export async function getDirtyFarmers(limit = 100) {
  const rows = await db()`
    ...
    player_stats AS (
      ...
      FROM (
        ...
        SELECT from_addr, 0.0, 0, 0, 0, 0.0, amount, 0.0
          FROM transfers WHERE op_type = 'DEX_SELL'
        UNION ALL
        SELECT to_addr,   0.0, 0, 0, 0, 0.0, 0.0, amount
          FROM transfers WHERE op_type = 'DEX_BUY'
      ) x
...
export async function getDirtyMarketCapHistory() {
  const rows = await db()`
    WITH
