...
export async function getPlayerLeaderboard(limit = 200, offset = 0, asOf = null) {
  ...
  return db()`
    ...
    FROM (
      ...
      SELECT from_addr, 0, 0, amount, 0, 0, timestamp
        FROM transfers WHERE kind IN ('SPEND','BURN') AND from_addr != ${ZERO_ADDR} AND from_addr != ALL(${DEX_POOLS}) AND timestamp <= ${cap}
      UNION ALL
      SELECT from_addr, 0, 0, 0, amount, 0, timestamp
        FROM transfers WHERE kind='TRANSFER'
          AND (op_type='DEX_SELL' OR to_addr = ANY(${DEX_POOLS}))
          AND from_addr != ${ZERO_ADDR} AND from_addr != ALL(${DEX_POOLS}) AND timestamp <= ${cap}
      UNION ALL
      SELECT to_addr,   0, 0, 0, 0, amount, timestamp
        FROM transfers WHERE kind='TRANSFER'
          AND (op_type='DEX_BUY' OR from_addr = ANY(${DEX_POOLS}))
          AND to_addr != ${ZERO_ADDR} AND to_addr != ALL(${DEX_POOLS}) AND timestamp <= ${cap}
    ) t
    GROUP BY addr
...
export async function getPlayerCount(asOf = null) {
  const { cap } = nowCap(asOf);
  const [row] = await db()`
    SELECT COUNT(DISTINCT to_addr) AS cnt FROM transfers WHERE kind='MINT' AND to_addr != ${ZERO_ADDR} AND timestamp <= ${cap}`;
  return Number(row?.cnt ?? 0);
}

export async function getPlayerStats(address, cycleStart = 0, asOf = null) {
  ...
  const [row] = await db()`
    ...
    FROM (
      SELECT amount AS earned, 1 AS ops, 0 AS spent, 0 AS dex_sold, 0 AS dex_bought, timestamp AS ts
        FROM transfers WHERE kind='MINT' AND to_addr = ${addr} AND timestamp <= ${cap}
      UNION ALL
      SELECT 0, 0, amount, 0, 0, timestamp
        FROM transfers WHERE kind IN ('SPEND','BURN') AND from_addr = ${addr} AND timestamp <= ${cap}
      UNION ALL
      SELECT 0, 0, 0, amount, 0, timestamp
        FROM transfers WHERE kind='TRANSFER' AND from_addr = ${addr}
          AND (op_type='DEX_SELL' OR to_addr = ANY(${DEX_POOLS})) AND timestamp <= ${cap}
      UNION ALL
      SELECT 0, 0, 0, 0, amount, timestamp
        FROM transfers WHERE kind='TRANSFER' AND to_addr = ${addr}
          AND (op_type='DEX_BUY' OR from_addr = ANY(${DEX_POOLS})) AND timestamp <= ${cap}
    ) t`;
  const [vault] = await db()`
    SELECT COALESCE(SUM(amount),0) AS total, COUNT(*) AS count FROM vault_payouts WHERE recipient = ${addr} AND timestamp <= ${cap}`;
  // Lifetime allocated rewards from the game's cycle_user_results (Supabase).
  // reward_amount_wei is the total allocated for the user across all
  // cycles — includes both already-claimed and not-yet-claimed amounts.
  // Convert from wei (USDM has 18 decimals) to USD float.
  const [vaultAlloc] = await db()`
    SELECT
      COALESCE(SUM(reward_amount_wei::numeric), 0) / 1e18 AS allocated,
      COUNT(*)::int                                       AS cycles,
      COALESCE(SUM(loadout_count), 0)::int                AS loadouts
    FROM cycle_rewards WHERE user_address = ${addr} AND created_at <= to_timestamp(${cap})`;
  // token_holders is mutable current-state; historical balance comes from
  // transfer deltas instead.
  const holder = asOf == null
    ? (await db()`SELECT balance FROM token_holders WHERE address = ${addr}`)[0]
    : (await db()`
        SELECT COALESCE(SUM(delta), 0) AS balance FROM (
          SELECT SUM(amount)  AS delta FROM transfers WHERE to_addr   = ${addr} AND timestamp <= ${cap}
          UNION ALL
          SELECT -SUM(amount) AS delta FROM transfers WHERE from_addr = ${addr} AND timestamp <= ${cap}
        ) t`)[0];
  ...
  const [hitsCycle] = await db()`
    ...
    FROM hits
    WHERE timestamp >= ${cycleStart} AND timestamp <= ${cap}
  ...
  return {
    ...
    vault_allocated:    Number(vaultAlloc?.allocated ?? 0),
    vault_cycles:       Number(vaultAlloc?.cycles    ?? 0),
    vault_loadouts:     Number(vaultAlloc?.loadouts  ?? 0),
...
export async function getPlayerActivity(address, limit = 100, offset = 0, asOf = null) {
  const addr = address.toLowerCase();
  const { cap } = nowCap(asOf);
  ...
  return db()`
    ...
    WHERE (t.to_addr = ${addr} OR t.from_addr = ${addr}) AND t.timestamp <= ${cap}
    ORDER BY t.timestamp DESC, t.log_index DESC LIMIT ${limit} OFFSET ${offset}`;
}

export async function getPlayerRecentMissionStats(address, asOf = null) {
  const addr    = address.toLowerCase();
  ...
  const rows = await db()`
    ...
    WHERE to_addr = ${addr}
      ...
      AND timestamp >= ${since24h} AND timestamp <= ${now}
    GROUP BY op_type`;
  return rows;
}

export async function getPlayerVaultPayouts(address, limit = 50, asOf = null) {
  const { cap } = nowCap(asOf);
  return db()`
    SELECT hash, log_index, timestamp, amount
    FROM vault_payouts WHERE recipient = ${address.toLowerCase()} AND timestamp <= ${cap}
    ORDER BY timestamp DESC LIMIT ${limit}`;
}

export async function getPlayerOpsBreakdown(address, asOf = null) {
  const addr = address.toLowerCase();
  const { cap } = nowCap(asOf);
...
export async function getPlayerInfluenceStats(address, asOf = null) {
  const addr = address.toLowerCase();
  const { cap } = nowCap(asOf);
  const [r] = await db()`
    SELECT
    ...
    FROM influence_transfers WHERE (to_addr = ${addr} OR from_addr = ${addr}) AND timestamp <= ${cap}`;
  return {
...
export async function getPlayerDailyHistory(address, asOf = null) {
  const addr = address.toLowerCase();
  const { cap } = nowCap(asOf);
