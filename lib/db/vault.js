...
export async function getVaultStats(asOf = null) {
  ...
  const [r] = await db()`
    ...
    FROM vault_payouts WHERE timestamp <= ${cap}`;
  return {
...
export async function getVaultCycleHistory(asOf = null) {
  const { cap } = nowCap(asOf);
  return db()`SELECT timestamp, amount FROM vault_payouts WHERE timestamp <= ${cap} ORDER BY timestamp ASC`;
}

export async function getVaultTopEarners(limit = 50, asOf = null) {
  const { cap } = nowCap(asOf);
  return db()`
...
export async function getVaultRecentPayouts(limit = 100, asOf = null) {
  const { cap } = nowCap(asOf);
  return db()`
    SELECT hash, log_index, timestamp, recipient, amount
    FROM vault_payouts WHERE timestamp <= ${cap} ORDER BY timestamp DESC, log_index DESC LIMIT ${limit}`;
}

// Returns each recipient's earliest payout timestamp — used to compute new recipients per cycle.
export async function getVaultFirstPayouts(asOf = null) {
  const { cap } = nowCap(asOf);
  return db()`SELECT MIN(timestamp) AS first_ts FROM vault_payouts WHERE timestamp <= ${cap} GROUP BY recipient`;
}
