...
import { GENESIS } from '../chain-constants.js';
const db = () => getDb();
...
export async function getCompanyStats(asOf = null) {
  // `companies` is mutable current-state — usable only for "now". Historical
  // asOf callers get event-derived totals plus null live-state fields; the
  // route fills activeCount from the demo-trades reconstruction.
  const capBlock = asOf == null ? 9007199254740991 : asOf - GENESIS;

  const [liveStats] = asOf == null ? await db()`
    SELECT
  ...
  return {
    ...
    activeCount:      liveStats ? Number(liveStats.active_count ?? 0) : null,
    autoTradeCount:   liveStats ? Number(liveStats.auto_trade_count ?? 0) : null,
    liquidatableCount:liveStats ? Number(liveStats.liquidatable_count ?? 0) : null,
    completableCount: liveStats ? Number(liveStats.completable_count ?? 0) : null,
...
export async function getEnhancedLeaderboard(limit = 1000, offset = 0, asOf = null) {
  const { cap } = nowCap(asOf);
  // The companies LEFT JOIN reads mutable current-state — meaningless at a
  // historical asOf, so its columns are nulled out in that case below.
  const rows = await db()`
    SELECT
      p.addr,
      p.earned,
  ...
  return rows.map(r => ({
    ...
    num_companies:   asOf == null ? Number(r.num_companies   ?? 0) : null,
    active_companies:asOf == null ? Number(r.active_companies ?? 0) : null,
    liq_companies:   asOf == null ? Number(r.liq_companies   ?? 0) : null,
    auto_off_active: asOf == null ? Number(r.auto_off_active  ?? 0) : null,
  }));
}
