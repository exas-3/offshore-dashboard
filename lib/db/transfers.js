...
export async function getRecentTransfers(limit = 100, offset = 0, asOf = null) {
  ...
  return db()`
    ...
    WHERE t.timestamp <= ${cap}
    ORDER BY t.timestamp DESC, t.log_index DESC LIMIT ${limit} OFFSET ${offset}`;
}

export async function getTotalTransferCount(asOf = null) {
  const { cap } = nowCap(asOf);
  const [row] = await db()`SELECT COUNT(*) AS cnt FROM transfers WHERE timestamp <= ${cap}`;
...
export async function getStats(asOf = null) {
  const { cap } = nowCap(asOf);
  const [mint] = await db()`
...
export async function getOpBreakdown(asOf = null) {
  const { cap } = nowCap(asOf);
  const rows = await db()`
