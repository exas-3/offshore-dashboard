...
export async function getPriceBaseline25h(asOf = null) {
  const { now, cap } = nowCap(asOf);
  const target = now - 24 * 3600;
  const [dirty] = await db()`
    SELECT dirty FROM price_snapshots
    WHERE dirty IS NOT NULL AND timestamp <= ${cap}
...
export async function getLatestTokenInfo(asOf = null) {
  const { cap } = nowCap(asOf);
  const [latest] = await db()`SELECT supply, holders FROM supply_snapshots WHERE timestamp <= ${cap} ORDER BY timestamp DESC LIMIT 1`;
  if (!latest) return { supply: null, holders: null };
  if (latest.holders == null) {
    const [withH] = await db()`SELECT holders FROM supply_snapshots WHERE holders IS NOT NULL AND timestamp <= ${cap} ORDER BY timestamp DESC LIMIT 1`;
    return { supply: latest.supply, holders: withH?.holders ?? null };
  }
  return { supply: latest.supply, holders: latest.holders };
}

export async function getLatestEthPrice(asOf = null) {
  const { cap } = nowCap(asOf);
  const [row] = await db()`SELECT price_usd FROM eth_price_snapshots WHERE timestamp <= ${cap} ORDER BY timestamp DESC LIMIT 1`;
  return row?.price_usd ?? null;
}

// As-of DIRTY price + INF op cost from the 60s price_snapshots tape —
// the demo-mode substitute for fetchDirtyPrice()/fetchLatestInfCost().
// The snapshot tape only starts 2026-05-13; earlier moments fall back to the
// last indexed DEX swap price (v_dex_swaps covers the window from 05-05).
export async function getPricesAt(asOf = null) {
  const { cap } = nowCap(asOf);
  let [dirtyRow] = await db()`
    SELECT dirty::float AS v FROM price_snapshots
    WHERE dirty IS NOT NULL AND timestamp <= ${cap}
    ORDER BY timestamp DESC LIMIT 1`;
  if (!dirtyRow) {
    [dirtyRow] = await db()`
      SELECT price_usdm_per_dirty::float AS v FROM v_dex_swaps
      WHERE price_usdm_per_dirty IS NOT NULL AND ts <= ${cap}
      ORDER BY ts DESC LIMIT 1`.catch(() => []);
  }
  const [infRow] = await db()`
    SELECT inf_cost::float AS v FROM price_snapshots
    WHERE inf_cost IS NOT NULL AND timestamp <= ${cap}
    ORDER BY timestamp DESC LIMIT 1`;
  return { dirty: dirtyRow?.v ?? null, infCost: infRow?.v ?? null };
}

export async function getSupplyHistory(hours = 24, asOf = null) {
  const { now } = nowCap(asOf);
  const since = now - hours * 3600;
  return db()`
    SELECT supply FROM supply_snapshots WHERE timestamp >= ${since} AND timestamp <= ${now} ORDER BY timestamp ASC`;
}

export async function getEthPriceHistory(hours = 24, asOf = null) {
  const { now } = nowCap(asOf);
  const since = now - hours * 3600;
  return db()`
    SELECT timestamp, price_usd FROM eth_price_snapshots WHERE timestamp >= ${since} AND timestamp <= ${now} ORDER BY timestamp ASC`;
}
...
// Historical holder count from transfer deltas — token_holders is a mutable
// current-state snapshot and can't answer "how many holders at T".
export async function getHolderCountAt(asOf = null) {
  const { cap } = nowCap(asOf);
  const [row] = await db()`
    SELECT COUNT(*) AS cnt FROM (
      SELECT addr FROM (
        SELECT to_addr AS addr, SUM(amount) AS delta FROM transfers WHERE timestamp <= ${cap} GROUP BY to_addr
        UNION ALL
        SELECT from_addr, -SUM(amount) FROM transfers WHERE timestamp <= ${cap} GROUP BY from_addr
      ) t GROUP BY addr HAVING SUM(delta) > 0.0001
    ) u`;
  return Number(row?.cnt ?? 0);
}

export async function getTopEarners(limit = 100, asOf = null) {
  const { cap } = nowCap(asOf);
  return db()`
...
export async function getDexActivity(since = 0, limit = 50, asOf = null) {
  const { cap } = nowCap(asOf);
  return db()`
    ...
    WHERE kind = 'TRANSFER'
      ...
      AND timestamp > ${since} AND timestamp <= ${cap}
    ORDER BY timestamp DESC, log_index DESC LIMIT ${limit}`;
}

export async function getWalletActivity(addresses, limit = 100, since = 0, asOf = null) {
  const { cap } = nowCap(asOf);
  return db()`
    SELECT hash, log_index, timestamp, from_addr, to_addr, amount, kind, op_type
    FROM transfers
