...
export async function getInfNetFlow24h() {
  ...
  const [inf, dirty, prices, latestPriceRow] = await Promise.all([
    db()`
      ...
      WHERE kind = 'MINT' AND timestamp >= ${start}
        AND NOT EXISTS (
    ...
    db()`
      ...
      WHERE dirty IS NOT NULL AND timestamp >= ${start}
      GROUP BY hr`,
    db()`SELECT dirty::float AS price FROM price_snapshots WHERE dirty IS NOT NULL ORDER BY timestamp DESC LIMIT 1`,
  ]);
