...
const HITS_ADDR = HITS.toLowerCase();
...
// Fetch factory DirtyPaid + FullCompletion + TradeExited events for a block range.
// Returns { companyMap, fullTxs, companyBlockMap } describing the per-trade context.
// The trade type itself is resolved by the caller via getCompanyTradeTypesFromStorage
// (slots 4/5 → duration → type), not from a factory event.
export async function fetchFactoryTradeContext(fromBlock, toBlock) {
  const fromHex = '0x' + fromBlock.toString(16);
  const toHex   = '0x' + toBlock.toString(16);
  const [payoutLogs, completeLogs, exitedLogs] = await Promise.all([
  ...
  // Bust path: liquidations.js synthesizes a MINT row using the E_EXITED
  // log_index, so key companyMap by that index too. Keeps partial-sweep
  // able to resolve PARTIALs from either origin (DirtyPaid or TradeExited).
  for (const l of exitedLogs ?? []) {
    const txHash = l.transactionHash.toLowerCase();
    if (!payoutTxs.has(txHash)) {
      const company = ('0x' + l.topics[1].slice(26)).toLowerCase();
      companyMap.set(txHash + ':' + parseInt(l.logIndex, 16), company);
      const bn = parseInt(l.blockNumber, 16);
      if (bn > (companyBlockMap.get(company) ?? 0)) companyBlockMap.set(company, bn);
    }
  }
  const fullTxs = new Set((completeLogs ?? []).map(l => l.transactionHash.toLowerCase()));
  return { companyMap, fullTxs, companyBlockMap, payoutTxs };
}
...
// Fetch HitResolved events from the HITS contract for a block range. Returns
// { hits: Array<{hash, logIndex, blockNum, timestamp, attacker, victim, company, mode, completionBps, fullPayout, stolen, kept, cost}>, byTx: Map<txHash, {attacker, victim, stolen, kept, cost}> }
// `byTx` is a quick lookup for the sync layer to re-tag MINTs by recipient.
export async function fetchHitEvents(fromBlock, toBlock) {
  const logs = await rpcPost('eth_getLogs', [{
    address:   HITS_ADDR,
    topics:    [E_HIT_RESOLVED],
    fromBlock: '0x' + fromBlock.toString(16),
    toBlock:   '0x' + toBlock.toString(16),
  }]).catch(() => []);
  const hits = [];
  const byTx = new Map();
  for (const l of logs ?? []) {
    if (!l?.topics?.[1] || !l?.topics?.[2] || !l?.data) continue;
    const raw = l.data.slice(2);
    // 6 32-byte words: mode, completionBps, fullPayout, stolen, kept, cost
    const word = (i) => BigInt('0x' + raw.slice(i * 64, (i + 1) * 64));
    const mode          = Number(word(0));
    const completionBps = Number(word(1));
    const fullPayout    = fromWei(word(2).toString());
    const stolen        = fromWei(word(3).toString());
    const kept          = fromWei(word(4).toString());
    const cost          = fromWei(word(5).toString());
    const hash          = l.transactionHash.toLowerCase();
    const attacker      = ('0x' + l.topics[1].slice(26)).toLowerCase();
    const victim        = ('0x' + l.topics[2].slice(26)).toLowerCase();
    const company       = l.topics[3] ? ('0x' + l.topics[3].slice(26)).toLowerCase() : '';
    ...
    hits.push({
      ...
      attacker, victim, company,
      mode, completionBps, fullPayout, stolen, kept, cost,
    });
    byTx.set(hash, { attacker, victim, stolen, kept, cost });
  }
  return { hits, byTx };
}
